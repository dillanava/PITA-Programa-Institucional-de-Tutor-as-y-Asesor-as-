...
if ($result->num_rows > 0) {
    ...
    $sql_insert = "INSERT INTO citas_eliminadas (id_citas, fecha, matricula, nempleado, tipo, id_citasN, status, hora, tutor, periodo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql_insert);
    $stmt->bind_param("isiiisisss", $row['id_citas'], $row['fecha'], $row['matricula'], $row['nempleado'], $row['tipo'],  $row['id_citasN'],   $row['status'], $row['hora'], $row['tutor'], $row['periodo']);
    $stmt->execute();
    $stmt->close();
