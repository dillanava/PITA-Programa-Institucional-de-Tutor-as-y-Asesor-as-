...
if (isset($_FILES['imagen_perfil']) && $_FILES['imagen_perfil']['error'] == 0) {
    ...
    $target_file = $target_dir . 'a' .  $_SESSION['user'] . ".jpg";

    // Eliminar la imagen anterior si existe
    $sql = "SELECT imagen_de_perfil FROM alumnos WHERE matricula = ?";
    ...
    if (move_uploaded_file($_FILES["imagen_perfil"]["tmp_name"], $target_file)) {
        ...
        $stmt = $conn->prepare("UPDATE alumnos SET imagen_de_perfil = ? WHERE matricula = ?");
