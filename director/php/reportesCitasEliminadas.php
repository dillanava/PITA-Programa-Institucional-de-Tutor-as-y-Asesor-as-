...
// Obtener información de citas eliminadas
$sql_citas = "SELECT id_citas, fecha, matricula, nempleado, descripcion, id_citasN, status, hora FROM citas_eliminadas";
...
$pdf->writeHTML('<br><h1 style="text-align: center; font-size: 20pt;">Reporte general de citas eliminadas</h1>'. '<br><h2 style="text-align: center">Profesor: <i>' . $nombreUsuario . '</i><h2><br><br>');


// Obtener información de citas eliminadas con INNER JOIN en nivelcitas
$sql_citas = "SELECT citas_eliminadas.id_citas, citas_eliminadas.fecha, citas_eliminadas.hora, citas_eliminadas.matricula, citas_eliminadas.nempleado, citas_eliminadas.id_citasN, citas_eliminadas.status, nivelcitas.nombre AS nivel_nombre FROM citas_eliminadas INNER JOIN nivelcitas ON citas_eliminadas.id_citasN = nivelcitas.id_citasN";
$result_citas = $conn->query($sql_citas);
...
$pdf->lastPage();
$pdf->Output('reporte_citas_eliminadas.pdf', 'I');
