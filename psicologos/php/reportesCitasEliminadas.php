...
if ($tutor == null || $tutor == '' || $idnivel != 3 || $nombreUsuario == '') {
...
$pdf->writeHTML('<br><h1 style="text-align: center; font-size: 20pt;">Reporte de citas eliminadas de profesor</h1>'. '<br><h2 style="text-align: center">Profesor: <i>' . $nombreUsuario . '</i><h2><br><br>');
        ...
        foreach ($row as $key => $cell) {
            if ($key === 'id_citasN') {
                ...
                if ((int)$cell === 1) {
                    ...
                    $cell = '<span style="background-color: #DDD;">No asistió</span>';
                }
            } elseif ($key === 'tipo') {
...
$sql_citas_psicologica_profesor = "SELECT citas_eliminadas.id_citas, citas_eliminadas.fecha, citas_eliminadas.hora, citas_eliminadas.matricula, citas_eliminadas.id_citasN, citas_eliminadas.status, nivelcitas.nombre AS nivel_nombre FROM citas_eliminadas INNER JOIN nivelcitas ON citas_eliminadas.id_citasN = nivelcitas.id_citasN INNER JOIN profesores ON citas_eliminadas.nempleado = profesores.nempleado WHERE nivelcitas.nombre = 'profesor' AND citas_eliminadas.nempleado = '$tutor'";
$result_citas_psicologica_profesor = $conn->query($sql_citas_psicologica_profesor);

// Obtener información de citas psicológicas de un profesor específico
$sql_citas_tutor = "SELECT citas_eliminadas.id_citas, citas_eliminadas.fecha, citas_eliminadas.hora, citas_eliminadas.matricula, citas_eliminadas.id_citasN, citas_eliminadas.status, tipo_problema.tipo_problema as tipo
FROM citas_eliminadas
INNER JOIN tipo_problema ON citas_eliminadas.tipo = tipo_problema.id_tipo_problema
WHERE citas_eliminadas.nempleado = '$tutor'";

$result_citas_2 = $conn->query($sql_citas_tutor);

// Encabezados de la tabla de citas
$header_citas_generartabla2 = array('ID', 'Fecha', 'Hora' , 'Matrícula', 'Tipo de cita', 'Status', 'Tipo de problema');
